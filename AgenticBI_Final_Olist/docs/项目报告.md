...
第三层是多智能体分析层。系统以 LangGraph `StateGraph` 为编排核心，所有 Agent 共享 `AgenticState` 状态对象。用户问题首先进入 Coordinator Agent，由协调器判断问题意图、生成结构化任务计划，并决定常规链路是否需要预测、NLP 和可视化。随后 SQL Agent 根据问题生成安全 SELECT 查询，并优先命中预聚合视图。Forecast Agent 基于 `mv_monthly_sales` 执行未来 6 周 GMV 预测；NLP Agent 从评论中抽取关键词、情感极性和主观性；Visualization Agent 根据可用结果生成图表。What-if Agent 和 Anomaly Agent 作为加分节点，用于模拟高差评卖家治理对评分的影响、扫描订单量骤降和准时率异常等风险；Decision Agent 最后综合 SQL 结果、预测结果、NLP 结果和诊断表摘要，形成面向运营团队的解释与建议。
...
在 Agent 框架方面，项目选用 LangGraph。相比线性函数调用，LangGraph 的优势是可以将多个 Agent 节点组织成状态图，并通过 `add_conditional_edges` 实现条件分支。项目中的图从 Coordinator 节点开始，进入 Analysis 节点后根据 `state.route` 决定是否进入 Forecast、NLP、Viz 和 Decision 节点；What-if 与 Anomaly 属于加分特种节点，主要由规则兜底关键词触发，并在可视化节点之后进入对应分支。描述性问题不触发 SARIMAX，评论问题才触发 NLP，模拟或异常问题才尝试进入对应加分节点。这样可以避免所有问题都执行全量分析，提高响应速度，也便于在报告和答辩中解释“为什么该问题调用了这些 Agent”。
...
在预测模型方面，项目采用 statsmodels SARIMAX 对 GMV 进行未来 6 周预测。模型输入来自 `mv_monthly_sales`，先将月度 GMV 序列插值为周频序列，再输出 6 个周度预测点和置信区间。Olist 数据的时间跨度约 25 个月，样本不算长，使用复杂深度学习模型并不合适。SARIMAX 对中小规模时间序列更易解释，也能输出置信区间，适合课程项目展示。由于输入来自预聚合视图，系统避免了每次预测前实时 JOIN orders 与 order_items。若历史数据不足，系统可回退到更简单的移动平均逻辑，保证预测节点不会阻塞整个图。
...
Coordinator Agent 是入口节点，负责理解用户问题并生成执行计划。它采用“LLM 优先、规则兜底”的策略。正常情况下，LLM 返回包含 intent、subtasks、route 和 context_rewrite 的 JSON。例如诊断型问题会生成 SQL、NLP、Decision 等子任务；预测型问题会设置 `forecast=true`；评论型问题会设置 `nlp=true`。如果 LLM 不可用或返回非 JSON，协调器会根据关键词触发规则路由；规则兜底中额外包含 What-if 与 Anomaly 关键词识别，从而保证系统在外部模型不稳定时仍能运行。
...
Forecast Agent 基于 `mv_monthly_sales` 进行未来 6 周 GMV 预测。它只在 route 中 `forecast=true` 时触发，避免非预测问题调用 SARIMAX。预测模块会先把月度 GMV 插值为周频序列，再生成未来 6 周预测结果。预测结果会保存为结构化表，既可供可视化 Agent 绘制趋势图，也可供 Decision Agent 解释趋势。
...
What-if Agent 作为加分功能，用于回答“如果将 Top 20 高差评卖家的商品统一下架，平台整体评分预估提升多少”这类问题。它从卖家绩效视图中筛选低评分卖家，使用订单量加权平均的方式估算移除这些卖家订单后的平台平均评分变化，并提示涉及卖家数、涉及订单数和评分提升幅度。当前实现不直接计算 GMV 损失，因此实际经营落地时仍需要结合卖家 GMV、替代供给和整改计划做进一步评估，避免给出过度简单的“直接下架”建议。

Anomaly Agent 用于扫描近期州级订单量骤降和准时率异常。它比较州级订单量的月环比变化，并识别准时率显著低于平台均值的州，将异常输出为风险等级和建议动作，体现系统从被动回答走向主动预警的能力。当前异常检测尚未直接计算差评率突升，差评风险主要通过诊断 SQL、卖家绩效视图和 NLP 结果进行分析。

Decision Agent 是最终汇总节点。它接收 SQL 查询摘要、诊断表摘要、预测结果和 NLP 结果，生成面向业务团队的解释与行动建议。What-if 与 Anomaly 节点会把结果写入共享状态，供界面或后续扩展使用；当前版本的 Decision prompt 主要汇总常规分析链路，没有把这两个加分节点的完整结构化结果强制拼入最终回答。若 LLM 没有返回文本，系统会基于已有表格生成 fallback 回答，例如直接指出某州准时率、Top 差评卖家或查询结果前五行。
...
综合以上结果，系统形成的经营建议可以归纳为五类。第一，物流侧优先改善北部和东北部高延迟州，通过中转仓、本地物流和加急配送提升准时率。第二，卖家侧对低评分且订单量不低的卖家设置 90 天整改期，持续低于阈值则降低曝光或下架。第三，品类侧对高差评品类实施商品描述审核、包装标准和售后 SLA。第四，支付侧围绕信用卡和分期设计高客单价转化活动，同时保留 boleto。第五，数据侧上线异常检测，持续扫描州级订单骤降和准时率突降，并结合卖家绩效与 NLP 结果定期复核差评风险，缩短问题发现周期。
...
第四个挑战是不同问题不应触发全部 Agent。最初的线性流程会让描述性问题也执行预测和 NLP，造成不必要等待。解决方案是通过 LangGraph 条件路由控制节点调用。描述性问题走 SQL、Viz、Decision；预测问题增加 Forecast；评论问题增加 NLP；What-if 和异常检测作为加分节点，主要通过规则兜底关键词触发并在可视化之后进入对应分支。当前实现中两个加分节点不会在同一次图执行中连续串联，因此报告和演示均按“单类加分能力分别验证”的口径说明。这样既提高效率，也让答辩中可以清晰解释系统“按问题类型选择最小必要路径”。
